...
// класа Vector,
// прави вектор од низа
class Vector {
  constructor(arr) {
    this.arr = arr;
  }

  // функција add прима вектор
  // и враћа нови вектор збира
  add(otherVector) {
    const oa = otherVector.arr;

    if (this.arr.length != oa.length) return;

    let res = [];
    for (let key in this.arr) res[key] = this.arr[key] + oa[key];
